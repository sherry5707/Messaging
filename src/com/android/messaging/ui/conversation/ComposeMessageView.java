...
public class ComposeMessageView extends LinearLayout
    ...
    private FrameLayout mSimSelectorIndicator;
    private ImageView mSimSelectorIndicatorIcon;
    ...
    protected void onFinishInflate() {
        ...
        mComposeEditText.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                if (mHost.shouldHideAttachmentsWhenSimSelectorShown()) {
                    Log.e("ComposeMessageView", "mComposeEditText.onClick: ");
                    hideSimSelector();
                }
            }
        });
        ...
        mSimSelectorIndicatorIcon = (ImageView) findViewById(R.id.sim_select_indicator_icon);
        mSimSelectorIndicator = (FrameLayout) findViewById(R.id.sim_select_indicator);
        mSimSelectorIndicator.setOnClickListener(new OnClickListener() {
            @Override
...
//                hideAttachmentsWhenShowingSims(shown);
    ...
    //if simPicker is visible,subject is gone
    private void hideShowSubjectEditor(final boolean simPickerVisible) {
        if (simPickerVisible) {
            hideSubjectEditor();
        } else {
            showSubjectEditor();
    ...
    private void updateVisualsOnDraftChanged() {
        ...
        if (selfSendButtonUri != null) {
            String index = selfSendButtonUri.getQueryParameter("i");
            if (!TextUtils.isEmpty(index)) {
                if (Integer.parseInt(index) == 1) {
                    mSimSelectorIndicatorIcon.setImageResource(R.drawable.ic_sim1);
                } else if (Integer.parseInt(index) == 2) {
                    mSimSelectorIndicatorIcon.setImageResource(R.drawable.ic_sim2);
                }
            }
            if (hasWorkingDraft && isDataLoadedForMessageSend()) {
                //UiUtils.revealOrHideViewWithAnimation(mSendButton, VISIBLE, null);
                mSendButton.setBackground(getResources().getDrawable(R.drawable.send_arrow_background));
                ...
                if (isOverriddenAvatarAGroup()) {
                    ...
                    // the send button.
                    UiUtils.revealOrHideViewWithAnimation(mSimSelectorIndicator, GONE, null);
                }
                ...
                //mSelfSendIcon.setImageResourceUri(selfSendButtonUri);
                if (isOverriddenAvatarAGroup()) {
                    UiUtils.revealOrHideViewWithAnimation(mSimSelectorIndicator, VISIBLE, null);
                }
                //UiUtils.revealOrHideViewWithAnimation(mSendButton, GONE, null);
                mSendButton.setBackground(getResources().getDrawable(R.drawable.send_arrow_background_gray));
    ...
    private void setSendButtonAccessibility(final int sendWidgetMode) {
        switch (sendWidgetMode) {
            case SEND_WIDGET_MODE_SELF_AVATAR:
                ...
                mSimSelectorIndicator.setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_NO);
                mSimSelectorIndicator.setContentDescription(null);
                mSimSelectorIndicator.setVisibility(GONE);
            ...
            case SEND_WIDGET_MODE_SIM_SELECTOR:
                ...
                mSimSelectorIndicator.setVisibility(VISIBLE);
                mSimSelectorIndicator.setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_YES);
                mSimSelectorIndicator.setContentDescription(getSimContentDescription());
    ...
    private void hideSimSelector() {
        Log.e("ComposeMessageView", "hideSimSelector: ");
    ...
    public void hideAllComposeInputs(final boolean animate) {
        Log.e("ComposeMessageView", "hideAllComposeInputs: ");
