...
public class ConversationFragment extends Fragment implements ConversationDataListener,
    ...
    private static final String CN_PHONE_NUMBER_PRE = "+86";
    ...
    private static final int SUB_PHONE_NUMBER_LENGTH = CN_PHONE_NUMBER_PRE.length();
    ...
    private final ActionMode.Callback mMessageActionModeCallback = new ActionMode.Callback() {
        ...
        public boolean onCreateActionMode(final ActionMode actionMode, final Menu menu) {
            ...
            if(data.getIsSms()){
                menu.findItem(R.id.action_favorites_delete).setVisible(data.isFavorited());
                menu.findItem(R.id.action_favorites_add).setVisible(!data.isFavorited());
            }else{
                menu.findItem(R.id.action_favorites_add).setVisible(false);
                menu.findItem(R.id.action_favorites_delete).setVisible(false);
            }
        ...
        public boolean onActionItemClicked(final ActionMode actionMode, final MenuItem menuItem) {
            ...
            switch (menuItem.getItemId()) {
                ...
                case R.id.action_favorites_add:
                    if (mSelectedMessage != null) {
                        add2Favorites(messageId);
                    }
                    return true;
                case R.id.action_favorites_delete:
                    if (mSelectedMessage != null) {
                        deleteMessageFromFavorites(messageId);
                    }
                    return true;
            }
            return false;
    ...
    private void selectMessage(final ConversationMessageView messageView,
        ...
        // TODO: 18-1-24 try popup menu
        showPopupMessageMenu();
//        mHost.startActionMode(mMessageActionModeCallback);
    }

    private void showPopupMessageMenu() {
        if(mSelectedMessage == null){
            throw new IllegalStateException("showPopupMessageMenu mSelectedMessage is null");
        }

        final ConversationMessageData data = mSelectedMessage.getData();
        boolean isIncoming = data.getIsIncoming();
        int gravity = isIncoming ? Gravity.LEFT : Gravity.RIGHT;
        PopupMenu popup = new PopupMenu(getContext(), mSelectedMessage, gravity);
        popup.inflate(R.menu.conversation_fragment_select_menu);
...
//                return true;
    ...
    void add2Favorites(final String messageId) {
        //1.step save
        mBinding.getData().save2Favorites(mBinding, messageId);
        //2. finish action mode
        mHost.dismissActionMode();
    }

    void deleteMessageFromFavorites(final String messageId) {
        //1.step delete
        mBinding.getData().deleteMessageFromFavorites(mBinding, messageId);
        //2. finish action mode
        mHost.dismissActionMode();
    }
...
//        UiUtils.setStatusBarColor(getActivity(), themeColor);
    ...
    public void updateActionBar(final ActionBar actionBar) {
        if (mComposeMessageView == null || !mComposeMessageView.updateActionBar(actionBar)) {
            ...
            final TextView conversationSubNameView =
                    (TextView) customView.findViewById(R.id.conversation_sub_title);
            ...
            if (!TextUtils.isEmpty(conversationName)) {
                ...
                String number = mBinding.getData().getParticipantPhoneNumber();
                String location = ContactUtil.getGeoDescription(getContext(), number);
                if (TextUtils.isEmpty(number) || TextUtils.isEmpty(location)) {
                    conversationSubNameView.setVisibility(View.GONE);
                } else {
                    conversationSubNameView.setVisibility(View.VISIBLE);

                    if (conversationName.replace(STR_SPACE, STR_NULL).equals(number)) {
                        conversationSubNameView.setText(location);
                    } else {
                        StringBuilder stringBuilder = new StringBuilder();
                        if (number.startsWith(CN_PHONE_NUMBER_PRE)) {
                            stringBuilder.append(number.substring(SUB_PHONE_NUMBER_LENGTH));
                        } else {
                            stringBuilder.append(number);
                        }
                        stringBuilder.append(STR_BAR);
                        stringBuilder.append(location.replace(STR_PROVINCE, STR_NULL).replace(STR_CITY, STR_NULL));
                        conversationSubNameView.setText(stringBuilder.toString());
                    }
                }
            } else {
