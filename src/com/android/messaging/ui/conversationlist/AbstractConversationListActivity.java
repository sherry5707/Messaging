...
import com.android.messaging.datamodel.action.ChangeTopChatAction;
...
public abstract class AbstractConversationListActivity  extends BugleActionBarActivity
    ...
    private int mMultipleSelectionNum;
    private UpdateSelectedNumListener selectedNumListener;
    private MultipleSelectModeListener multiModeListener;
    ...
    protected void startMultiSelectActionMode() {
        ...
        if (multiModeListener != null) {
            multiModeListener.updateToolBar(isInConversationListSelectMode());
        }
    }
    ...
    @Override
    public void onActionBarDelete(final List<String> conversationIds) {
        ArrayMap<String, SelectedConversation> conversations = prepareDataDoAction(conversationIds);
                ...
                .setPositiveButton(R.string.delete_conversation_confirmation_button,
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(final DialogInterface dialog,
                                    final int button) {
                                for (final SelectedConversation conversation : conversations.values()) {
    ...
    @Override
    public void onActionBarTopChat(ArrayList<String> conversationIds, boolean isTopChatOn) {
        ChangeTopChatAction.markAsTopChat(conversationIds, isTopChatOn);

        final int textId = isTopChatOn ?
                R.string.top_message : R.string.cancel_top_message;
        final String message = getResources().getString(textId, conversationIds.size());
        UiUtils.showSnackBar(this, findViewById(android.R.id.list), message,
                null /* undoRunnable */,
                SnackBar.Action.SNACK_BAR_UNDO, mConversationListFragment.getSnackBarInteractions());
    ...
    public void selectAllConversations(){
        ...
        mConversationListFragment.selectAllConversations(multiSelectActionMode);
    }

    public void exitSelectAllConversations(){
        final MultiSelectActionModeCallback multiSelectActionMode =
                (MultiSelectActionModeCallback) getActionModeCallback();
        mConversationListFragment.exitSelectAllConversations(multiSelectActionMode);
    }

    public ArrayMap<String, SelectedConversation> prepareDataDoAction(List<String> tempSelectItems) {
        return mConversationListFragment.prepareSelectedData(tempSelectItems);
    }
    ...
    public void onConversationClick(final ConversationListData listData,
        ...
        if (isInConversationListSelectMode()) {
            ...
            mMultipleSelectionNum = multiSelectActionMode.getSelectedConversationsNum();
            if (selectedNumListener != null) {
                selectedNumListener.setMultipleSize(mMultipleSelectionNum);
            }
            mConversationListFragment.updateUi();
        } else {
    ...
    /**
     * for give selected number to ConversationListActivity
     */
    public interface UpdateSelectedNumListener {
        public void setMultipleSize(int number);
    }

    public void setSelectedNumListener(UpdateSelectedNumListener listener) {
        this.selectedNumListener = listener;
    }

    public int getConversationsNumber() {
        return mConversationListFragment.getCursorCount();
    }

    /**
     * when in MultipleSelectMode ,update ToolBar
     */
    public interface MultipleSelectModeListener {
        public void updateToolBar(boolean isMultiMode);
    }

    public void setMultiSelectModeListener(MultipleSelectModeListener listener) {
        this.multiModeListener = listener;
    }
    ...
    protected void markAsReadForAll() {
        mConversationListFragment.markAsRead();
    }
