...
public class MultiSelectActionModeCallback implements Callback {
    ...
    private int mTopChatCount;          //it shows the topchat count in selected conversations not all the conversations

    public interface Listener {
        void onActionBarDelete(List<String> conversationIds);
        ...
        void onActionBarTopChat(ArrayList<String> conversationIds,
                                     boolean isTopChatOn);
    ...
    static class SelectedConversation {
        ...
        private final CharSequence participantLookupKey;
        ...
        public SelectedConversation(ConversationListItemData data) {
            ...
            isTopChat = data.getIsTopChat();
        }
    }

    private final ArrayMap<String, SelectedConversation> mSelectedConversations;

    private final ArrayList<String> mTempSelectItems;
...
//    private MenuItem mNotificationOffMenuItem;
    private MenuItem mTopChatOnMenuItem;
    private MenuItem mTopChatOffMenuItem;
    private MenuItem mDeleteMenuItem;
    private boolean mHasInflated;

    public int getSelectedConversationsNum(){
        return mTempSelectItems.size();
    }

    public MultiSelectActionModeCallback(final Listener listener) {
        mListener = listener;
        mSelectedConversations = new ArrayMap<>();
        mTempSelectItems = new ArrayList<>();
    }
...
//        mNotificationOnMenuItem = menu.findItem(R.id.action_notification_on);
        mTopChatOffMenuItem = menu.findItem(R.id.action_top_off);
        mTopChatOnMenuItem = menu.findItem(R.id.action_top_on);
    ...
    public boolean onActionItemClicked(ActionMode actionMode, MenuItem menuItem) {
        switch(menuItem.getItemId()) {
            case R.id.action_delete:
                mListener.onActionBarDelete(mTempSelectItems);
                return true;
...
//                return true;
            case R.id.action_top_off:
                mListener.onActionBarTopChat(mTempSelectItems, false);
                return true;
            case R.id.action_top_on:
                mListener.onActionBarTopChat(mTempSelectItems, true);
                return true;
    ...
    public void toggleSelect(final ConversationListData listData,
        ...
        final boolean isTopChat = conversationListItemData.getIsTopChat();
        ...
        if (mTempSelectItems.contains(id)) {
            if (isTopChat) {
                mTopChatCount--;
            }
            mTempSelectItems.remove(id);
        } else {
            if (isTopChat) {
                mTopChatCount++;
            }
            mTempSelectItems.add(id);
        }

        if (mTempSelectItems.isEmpty()) {
            mListener.onActionBarHome();
        } else {
            updateActionIconsVisiblity();
        }
    ...
    public void toggleSelect(Cursor cursor) {
        mTopChatCount = 0;
        if (cursor == null) {
            mSelectedConversations.clear();
        ...
        cursor.moveToFirst();
        do {
            String id = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ConversationColumns._ID));
            int top_chat = cursor.getInt(cursor.getColumnIndex(DatabaseHelper.ConversationColumns.TOP_CHAT));
            if (top_chat == 1) {
                mTopChatCount++;
            }
            if (!mTempSelectItems.contains(id)) {
    ...
    public boolean isSelected(final String selectedId) {
        ...
        return mTempSelectItems.contains(selectedId);
    }
...
//        }
        ...
        if (mTempSelectItems.size() > 0) {
            mDeleteMenuItem.setVisible(true);
        } else {
            mDeleteMenuItem.setVisible(false);
            mTopChatOffMenuItem.setVisible(false);
            mTopChatOnMenuItem.setVisible(false);
            return;
        }
...
//        mUnarchiveMenuItem.setVisible(hasCurrentlyArchived);
        ...
        // If we have topchat off conversations we show on button, if we have topchat on
        // conversation we show off button. We can show both if we have a mixture.
        boolean noTopChat = false;
        if (mTempSelectItems.size() == mSelectedConversations.size()) {
            final Iterable<SelectedConversation> conversations = mSelectedConversations.values();
            // as long as it has a conversation is not topChat,the icon should be mTopChatOnMenuItem visible.
            for (final SelectedConversation conversation : conversations) {
                if (!conversation.isTopChat) {
                    noTopChat = true;
                    break;
                }
            }
            mTopChatOnMenuItem.setVisible(noTopChat);
            mTopChatOffMenuItem.setVisible(!noTopChat);
        } else {
            if (mTempSelectItems.size() == mTopChatCount) {
                mTopChatOffMenuItem.setVisible(true);
                mTopChatOnMenuItem.setVisible(false);
            } else {
                mTopChatOffMenuItem.setVisible(false);
                mTopChatOnMenuItem.setVisible(true);
            }
        }
    }
}
