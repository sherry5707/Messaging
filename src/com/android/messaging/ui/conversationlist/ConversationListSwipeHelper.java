...
public class ConversationListSwipeHelper implements OnItemTouchListener {
    ...
    private static final float PERCENTAGE_OF_WIDTH_TO_SWIPE = 0.2f;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                if (hasValidGestureSwipeTarget()) {
                    ...
                    final boolean farEnough = isTargetSwipedFarEnough(swipeDirection);
    ...
    private void animateDismiss(final ConversationListItemView itemView,
        ...
        final float animateTo = (swipeDirection == SWIPE_DIRECTION_RIGHT) ?
                mRecyclerView.getWidth() / 2 : -mRecyclerView.getWidth() * PERCENTAGE_OF_WIDTH_TO_SWIPE;
        final long duration;
        if (velocityX != 0) {
...
//                }
     ...
     */
    private boolean isTargetSwipedFarEnough(int swipeDirection) {
        final float velocityX = getLastComputedXVelocity();
        ...
        return (velocityX >= 0) == (translationX > 0) // Right direction.
                && Math.abs(translationX) >
                ((swipeDirection == SWIPE_DIRECTION_RIGHT) ? PERCENTAGE_OF_WIDTH_TO_SWIPE
