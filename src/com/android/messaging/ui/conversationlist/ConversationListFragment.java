...
import com.android.messaging.datamodel.action.MarkAsUnreadAction;
...
public class ConversationListFragment extends Fragment implements ConversationListDataListener,
    ...
    public void onCreate(final Bundle bundle) {
        ...
        //searchView clicked,this is callback
        mAdapter.setSearchViewClickListener(new ConversationListAdapter.OnSearchViewClickListener() {
            @Override
            public boolean onClick() {
                if (getActivity() instanceof ConversationListActivity) {
                    return ((ConversationListActivity) getActivity()).startSearch();
                }
                return false;
            }
        });
        ...
        if (getActivity() instanceof ConversationListActivity) {
            ((ConversationListActivity) getActivity()).setSearchMsgListener(
                    new ConversationListActivity.SearchMsgListener() {
                        @Override
                        ...
                        @Override
                        public void onSearchMsg(String searchString) {
                            if (mListBinding != null) {
                                mListBinding.getData().startSearchMsg(searchString);
                                mAdapter.setInSearchMode(searchString);
                        ...
                        public void onExitSearch() {
                            if (mListBinding != null) {
                                mListBinding.getData().exitSearchMsg();
                                mAdapter.setInSearchMode(null);
    ...
    public void onConversationListCursorUpdated(final ConversationListData data,
                                                final Cursor cursor, FavoritesData favoritesData) {
        mListBinding.ensureBound(data);
        mAdapter.setFavorites(favoritesData);
        final Cursor oldCursor = mAdapter.swapCursor(cursor);
        //if conversationListActivity exit search mode,the adapter should recalculate the position and notifyDataSetChanged.
        if(Factory.get().getInSearchMode()==Factory.NOT_INSEARCH_MODE) {
            mAdapter.setSearchviewVisible(true);
        }
    ...
    @Override
    public void isAllReadConversations(boolean isAllRead) {
        mIsAllRead = isAllRead;
/*        if (mMarkAsRead != null) {
            mMarkAsRead.setTitle(isAllRead ? R.string.mark_as_unread_for_all : R.string.mark_as_read_for_all);
        }*/
    }
    ...
    public void onCreateOptionsMenu(final Menu menu, final MenuInflater inflater) {
        ...
        //mMarkAsRead = menu.findItem(R.id.action_mark_as_read);
    }
    ...
    public ViewPropertyAnimator dismissFab() {
        ...
        final ViewGroup.MarginLayoutParams lp =
                (ViewGroup.MarginLayoutParams) mStartNewConversationButton.getLayoutParams();
    ...
    public void selectAllConversations(MultiSelectActionModeCallback multiSelectActionMode){
        multiSelectActionMode.toggleSelect(mAdapter.getCursor());
        updateUi();
    }

    public void exitSelectAllConversations(MultiSelectActionModeCallback multiSelectActionMode){
        multiSelectActionMode.toggleSelect(null);
        updateUi();
    }

    public ArrayMap<String, MultiSelectActionModeCallback.SelectedConversation> prepareSelectedData(List<String> tempSelectIds) {
        Cursor cursor = mAdapter.getCursor();
        if (cursor == null || tempSelectIds.size() == 0) {
            return null;
        }
        ArrayMap<String, MultiSelectActionModeCallback.SelectedConversation> selectedConversations = new ArrayMap<>();
        ConversationListItemData itemData = new ConversationListItemData();
        for (String id : tempSelectIds) {
            cursor.moveToFirst();
            do {
                if (id.equals(cursor.getString(cursor.getColumnIndex(DatabaseHelper.ConversationColumns._ID)))) {
                    itemData.bind(cursor);
                    selectedConversations.put(id, new MultiSelectActionModeCallback.SelectedConversation(itemData));
                    break;
                }
    ...
    public void markAsRead(){
        ...
        do {
            id = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ConversationColumns._ID));
            conversationIds.add(id);
        } while (cursor.moveToNext());
        if (mIsAllRead) {
            MarkAsUnreadAction.markAsUnRead(conversationIds);
        } else {
            MarkAsReadAction.markAsRead(conversationIds);
        }
    }
