...
public class BugleDatabaseOperations {
    ...
    public static void updateConversationMetadataInTransaction(final DatabaseWrapper dbWrapper,
        ...
        int unreadCount = getUnreadMessageCount(dbWrapper, conversationId);
        values.put(ConversationColumns.UNREAD_COUNT, unreadCount);
    ...
    @DoesNotRunOnMainThread
    public static void updateConversationTopChatStatusInTransaction(final DatabaseWrapper dbWrapper,
            final String conversationId, final boolean isArchived) {
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
        final ContentValues values = new ContentValues();
        values.put(ConversationColumns.TOP_CHAT, isArchived ? 1 : 0);
        updateConversationRowIfExists(dbWrapper, conversationId, values);
    }
    ...
    @DoesNotRunOnMainThread
    public static void updateConversationListView(final DatabaseWrapper dbWrapper,
                                                 final String conversationId){
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
        final ContentValues values = new ContentValues();
    ...
    @DoesNotRunOnMainThread
    public static int getUnreadMessageCount(final DatabaseWrapper dbWrapper,
            final String conversationId) {
        int count = 0;
        Assert.isNotMainThread();
        Cursor cursor = null;
        try {
            cursor = dbWrapper.query(DatabaseHelper.MESSAGES_TABLE,
                    MessageData.getProjection(), MessageColumns.CONVERSATION_ID + "=?"
                            + " AND " + MessageColumns.READ + "=?",
                    new String[] { conversationId, "0" }, null, null, null);
            if (cursor!=null) {
    ...
    @DoesNotRunOnMainThread
    public static void markMessageFavorite(final DatabaseWrapper dbWrapper,
                                           final String messageId) {
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
        //  Update message row
        final ContentValues values = new ContentValues();
        values.put(MessageColumns.FAVORITES, 1);
        updateMessageRowIfExists(dbWrapper, messageId, values);
    }

    @DoesNotRunOnMainThread
    public static void cancelMarkMessageFavorite(final DatabaseWrapper dbWrapper,
                                           final String messageId) {
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
        //  Update message row
        final ContentValues values = new ContentValues();
        values.put(MessageColumns.FAVORITES, 0);
        updateMessageRowIfExists(dbWrapper, messageId, values);
    }

    @DoesNotRunOnMainThread
    public static void saveFavoriteDB(final DatabaseWrapper dbWrapper,
                                      final MessageData message) {
        Assert.isNotMainThread();
        ...
        String content = null;
        for (final MessagePartData part : message.getParts()) {
            if(message.getMessageId().equals(part.getMessageId())){
                content = part.getText();
                break;
            }
        }
        if(TextUtils.isEmpty(content)){
            throw new IllegalStateException("saved message text is null, please double check this");
        }
        values.put(DatabaseHelper.FavoritesColumns.CONTENT, content);
        values.put(DatabaseHelper.FavoritesColumns.SENT_TIMESTAMP, message.getSentTimeStamp());
        values.put(DatabaseHelper.FavoritesColumns.RECEIVED_TIMESTAMP, message.getReceivedTimeStamp());
        values.put(DatabaseHelper.FavoritesColumns.MESSAGE_ID, message.getMessageId());
        values.put(DatabaseHelper.FavoritesColumns.CONVERSATION_ID, message.getConversationId());

        //from conversation
        ConversationListItemData conversationListItemData =
                ConversationListItemData.getExistingConversation(dbWrapper, message.getConversationId());
        values.put(DatabaseHelper.FavoritesColumns.FULL_NAME, conversationListItemData.getName());
        values.put(DatabaseHelper.FavoritesColumns.SEND_DESTINATION,
                conversationListItemData.getOtherParticipantNormalizedDestination());
        values.put(DatabaseHelper.FavoritesColumns.PROFILE_PHOTO_URI,
                conversationListItemData.getIcon());
        values.put(DatabaseHelper.FavoritesColumns.PARTICIPANT_CONTACT_ID,
                conversationListItemData.getParticipantContactId());
        values.put(DatabaseHelper.FavoritesColumns.PARTICIPANT_LOOKUP_KEY,
                conversationListItemData.getParticipantLookupKey());

        //get sim and subId
        Cursor cursor = null;
        String participantId = null;
        try {
            cursor = dbWrapper.query(DatabaseHelper.CONVERSATION_PARTICIPANTS_TABLE,
                    null, ConversationParticipantsColumns.CONVERSATION_ID + "=?",
                    new String[] { conversationListItemData.getConversationId() }, null, null, null);
            Assert.inRange(cursor.getCount(), 0, 1);
            if (cursor.moveToFirst()) {
                participantId = cursor.getString(cursor.getColumnIndexOrThrow(ConversationParticipantsColumns.PARTICIPANT_ID));
            }
        } finally {
            if (cursor != null) {
                cursor.close();
        ...
        if(TextUtils.isEmpty(participantId)){
            throw new IllegalStateException("saved participantId is null, please double check this");
        }
        ParticipantData participantData = ParticipantData.getFromId(dbWrapper, participantId);
        values.put(DatabaseHelper.FavoritesColumns.SIM_SLOT_ID, participantData.getSlotId());
        values.put(DatabaseHelper.FavoritesColumns.SUB_ID, participantData.getSubId());

        //save current time
        values.put(DatabaseHelper.FavoritesColumns.FAVORITES_SAVED_TIME, System.currentTimeMillis());

        Assert.isTrue(dbWrapper.getDatabase().inTransaction());

        dbWrapper.insert(DatabaseHelper.FAVORITES_TABLE, null, values);
    }

    @DoesNotRunOnMainThread
    public static void deleteMessageFromFavoriteDB(final DatabaseWrapper dbWrapper,
                                                   final String messageId) {
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());

        dbWrapper.delete(DatabaseHelper.FAVORITES_TABLE,
                DatabaseHelper.FavoritesColumns.MESSAGE_ID + " =?",
